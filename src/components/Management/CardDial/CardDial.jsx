...
export default function CardDial({credit_id,campain_id}){
    ...
    const handlerHangup = async () => {
        ...
        if (recorderRef.current) {
            ...
            recorderRef.current.addEventListener('dataavailable', async (e) => {
                ...
                const fileName = `collecta_${timestamp}.webm`;
                const base = HandleBlobToFile(e.data, fileName, 'audio/webm');
        ...
        if (streamerRef.current) {
            streamerRef.current.getTracks().forEach(track => track.stop());
        }
    };

    useEffect(() => {
        store_call.setInfoCall({
            credit_id,
            campain_id
        });
    ...
    return (
        <div className={`CardDial ${(store_call.in_call) ? 'CardDial--incall' : ''}`}>
            ...
            <p className="CardDial__phone">{(store_call.phone_number!=='') ? `${store_call.message_status} ${store_call.phone_number}` : ''}</p>
            
            <div className="CardDial__actions">
                {
                    (!store_call.in_call)
                    ?
                        <>
                            <label
                                ...
                                onClick={async ()=>{ await handlerDial('PBX')}}
                            >
                                <PhoneForwarded size={18} />
                            </label>
                            <label
                                className="CardContact__item CardContact__item--whatsappcall"
                                onClick={async ()=>{ await handlerDial('WA')}}
                            >
